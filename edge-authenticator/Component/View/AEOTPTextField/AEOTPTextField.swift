...
public class AEOTPTextField: UITextField {
    ...
    public override func becomeFirstResponder() -> Bool {
        ...
        if textCount < digitLabels.count {
            moveCursor(to: textCount)
        } else {
...
private extension AEOTPTextField {
    ...
    private func moveCursor(to index: Int) {
        let targetLabel = digitLabels[index]
        cursorView.isHidden = false
        let cursorWidth: CGFloat = 2
        let cursorHeight: CGFloat = targetLabel.font.lineHeight
        
        cursorView.frame = CGRect(
            x: targetLabel.frame.midX - cursorWidth/2,
            y: targetLabel.frame.midY - cursorHeight/2,
            width: cursorWidth,
            height: cursorHeight
        )
    }
